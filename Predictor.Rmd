...
<!--Ben's to do list for each tournament
* Entry spreadsheet (including true results)
  * adjust entry spreadsheet to match tournament fixtures
  * check entry spreadsheet macro is to the correct row number
  * change all dates and logos to current tournament
  * save 7x placeholder entries and trueresults sheet with completed scores
  * save a random control entry sheet
  * save an AI generated predictions sheet?
  * save a Chris Sutton predictions sheet? This requires more admin as it won't be all at once!
  * find online free predictions such as https://oddsline.io/world-cup-predictions-today/

* rmd script
  * Change all dates to appropriate dates in script
  * test run script with placeholders and trueresults sheets
  * add/remove team specific plots? or keep wales in because its funnier.
  * switch between setup text and live text at the top of the html output
  * change whatsapp group name on picture (can just reuse the same code each time)

* shinyapp embed
  * republish resultsapp.r *after* generating AllResults.rds in rmd script
  * this will need doing twice, once with placeholder test and once when all entries have been submitted

* email account
  * check and adjust email rules for benspredictorgame@gmail.com account
  * attachments are not natively part of templated replies so reupload to google drive and refresh the attachment link
  
  * message to send out via whatsapp/email to potential players:
  For those of you that have played before, this is the message you've been waiting for. For those of you that are predictor newbies, get excited.
  
  The world cup 2026 predictor game is here!
  
  Key points are: predict group stage games at the world cup. Get points for how right you are and compete against people that *think* they know better. £5 entry fee that goes in a pot for the top predictors. Deadline is 9th June for entries. Its a bumper year with 72 (thanks FIFA/Infantino) group games so consider this fair notice to get your entry sorted.
  
  Go to https://bengillard.github.io/BensPredictorGame/ for all the information about the game. Or if you want to get stuck in straight away, email benspredictorgame@gmail.com with the subject line "i want to play".
  
  
  #game day checklist!
    * run a full simulation with random trueresults
      * check leaderboard to make sure every player has at least one correct score and at least correct result. Change truepredictions if needed to match a player's score. This is essential to make sure the formatting of entries is correct and == equations for accurate scores work as they should.
    * check plots for england goals scores. This will show any errors with names (if not filled out they will be NA) and the tie break question (double check entry spreadsheet to see if a zero is real or just missing data).
    * clear the test trueresults. You will have to leave the first results as 0-0 otherwise the code breaks
    * run app.r for embedded prediction checker and publish. This needs to be done before commit/push to github pages of main html
    * send out "We are LIVE!" email draft to all players (using tracker.xlsx for address list)
...
TrueResults<- readxl::read_excel("PredictorTrueResults.xlsm")[7:78, 5:8] #change dimensions depending on number of games
...
entries<- list.files("Entries_Proper/", full.names = TRUE)
...
predictions<- predictionsRoot[7:78, 5:8] #change dimensions depending on number of games
colnames(predictions)<- c("Home", "HomeScore", "AwayScore", "Away")
predictions$HomeScore<- gsub(x = predictions$HomeScore, pattern = ".0", replacement = "") #sometimes import includes decimal point in home and away scores which messes with the code for correct scores. Correct it here to make sure format works
predictions$AwayScore<- gsub(x = predictions$AwayScore, pattern = ".0", replacement = "") 
...
totalplayers<- length(Scores)-4 #minus randomcontrolentry, ChatGPT, ESPN/MickeyMouse, Claude
```

**Welcome to the World Cup 2026 Predictor Championship!** The aim of the game is simple, predict the scores for all of the group stage games at this year's tournament. Each correct *score* earns 3 points. Each correct *result* earns 1 point.

<!--setup text

**Follow this [link](https://github.com/BenGillard/BensPredictorGame/blob/main/Predictor.xlsm?raw=true) to download the entry spreadsheet or email benspredictorgame@gmail.com with the subject "I want to play!".** Please send the completed spreadsheet file with the subject line "Predictor 2026" to benspredictorgame@gmail.com. Then forward this website to all your friends so they can get involved too. **Deadline is 10pm on 9th June** ahead of the first fixture on 11th June so get your entry in quick!
...
If you want some inspiration, see this [research note](https://klementoninvesting.substack.com/p/fifa-world-cup-predictions-2026) from an economist who is very good at predicting football tournament champions.

Everything below is using test results to make sure everything is working properly! Once all entries have been submitted, the webpage will be updated with the real stuff.
-->
...
We have **`r totalplayers` players** involved so we have a healthy kitty to pay out for the **top 6 players. £`r round(x = totalplayers*5*0.44, digits = 3)` goes to the top dog. £`r round(x = totalplayers*5*0.24, digits = 3)` for the top loser. £`r round(x = totalplayers*5*0.14, digits = 3)` for the best of the rest. And £`r round(x = totalplayers*5*0.06, digits = 3)` for 4th, 5th, and 6th places.** *Although the bragging rights remain the most valuable prize.*

___

On the [Leaderboard] you'll see some extra players included in <span style="color: red;">red</span>. The <span style="color: red;">*RandomControlEntry*</span> predictions were generated randomly using the macro included in the spreadsheet. So if you have fewer points than RandomControlEntry, any thought you put into your entry was an utter waste of time. There are also 2 AI generated entries (<span style="color: red;">*ChatGPT*</span> and <span style="color: red;">*Claude*</span>) that were completed a week before the tournament started to show how powerful the computers are - although they both originally predicted scores for teams that didn't even qualify. [ESPN](https://www.espn.co.uk/football/story/_/id/48962628/world-cup-predictions-picking-winner-every-game-entire-tournament) have predicted all the group stage and are in the table as <span style="color: red;">*MickeyMouse*</span> in respect to their overlords.

Go to [Results Tracker] for a more in depth look at which games scored you points.

Forgot what you put for an upcoming game? Want to see what your bitter rival predicted too? Go to the [Entry Reminder] section and use the drop down to find full predictions for a specific player. Or if you want a general idea of the split of opinion for the next few matches go to [Upcoming Games].

To see players who have been immortalised by victory in previous tournaments, see the [Predictor Hall of Fame].

**There is a Predictor Whatsapp group! It's the best place to gloat/cry about your guesses. Click [here](https://chat.whatsapp.com/DlxhlOMRrhD1AbIVlo6Tk6) to join the group. Or scan the QR code below.**
...
#date information
ResultsSoFar<- readxl::read_excel("PredictorTrueResults.xlsm")[7:78, c(3,5:8)] #change depending on the number of fixtures
...
library(kableExtra)
ResultsSoFar[!is.na(ResultsSoFar[,3]),] %>% kbl(caption = "Soccer World Cup 2026 Results so far", align = "c", row.names = FALSE, col.names = NULL, linesep = "") %>% kable_classic(html_font = "Cambria") %>% kable_styling("striped", font_size = 10)
```
...
How things stand after **`r sum(!is.na(ResultsSoFar[,3]))` games out of `r nrow(Results$RandomControlEntry)`**. At the end of the game (Match Day 72(!) on 28th June 2026), winners will be contacted by email for their details so they can receive their hard fought winnings.

Correct games shows how many games you have scored points from (correct scores and results combined).
...
LeaderBoard<- cbind(LeaderBoard, CorrectGames = paste0(format(x = (LeaderBoard$CorrectScore + LeaderBoard$CorrectResult)/length(unlist(Scores$RandomControlEntry))*100, digits = 1),"%"))
...
kbl(LeaderBoard, caption = "Table of Champions", align = "l", row.names = TRUE) %>% kable_classic(html_font = "Cambria") %>%  row_spec(1, bold = T, background = "#c9b037") %>% row_spec(2, bold = T, background = "#b4b4b4") %>% row_spec(3, bold = T, background = "#ad8a56") %>% row_spec(4:6, bold = T, background = "#3CB371") %>% row_spec(which(LeaderBoard$Player == "RandomControlEntry"), italic = T, color = "red") %>% row_spec(which(LeaderBoard$Player == "ChatGPT"), italic = T, color = "red") %>% row_spec(which(LeaderBoard$Player == "Claude"), italic = T, color = "red") %>% row_spec(which(LeaderBoard$Player == "MickeyMouse"), italic = T, color = "red") %>% column_spec(column = 3, border_left = TRUE, border_right = TRUE)
```

> When 2 players have the same points tally, correct scores are taken into account. If players are still equal, the tie break question "How many total goals (including knockout games but excluding penalty shoot outs) will England score at the 2026 World Cup?" will come into play. Closest wins.
...
par(mar=c(7,4,4,4))
TrueGoals<- TrueResults %>% filter(Home == "England") %>% summarize(sum = sum(as.numeric(HomeScore))) + TrueResults %>% filter(Away == "England") %>% summarize(sum = sum(as.numeric(AwayScore)))
...
barplot(sort(unlist(AllGoals), decreasing = TRUE), main = "Tie Breaker: England Goals scored", col = c("red", "white"), las = 2, cex.names = 0.7)
abline(h = AllGoals$TrueGoals, lty = 2, col = "red")

#barplot(unlist(AllGoals[which(names(AllGoals) %in% c("TrueGoals", "Player1", "Player2"))]), main = "Tie Breaker: 3rd place", col = c("red", "white"), las = 2, cex.names = 0.7)
```
...
WinnerCount<- sort(sapply(paste0(".",unique(Results$CorrectResults$Home)), function (x) sum(grepl(x = names(unlist(WinnerCount)), pattern = x))))

WinnerCount<- WinnerCount[c(1:10,(length(WinnerCount)-10):length(WinnerCount))] #filter for top and bottom teams

barplot(WinnerCount, horiz = TRUE, las = 1, cex.names = 0.4, col = viridis::rocket(24), main = "Least and most fancied teams in this years tournament", xlab = "Number of wins predicted")
...
More specifically, how many times does each player think selected nations will win in the group stage (more teams available on request)? How many boring draws do people think we'll suffer through? Alternatively, are players expecting a goalfest?

```{r specificTeams, fig.show='hold', warning=FALSE, out.width='50%'}
par(mar=c(7,4,4,4))
#england wins
barplot(sort(sapply(names(Results[1:length(Results)]), function(x) sum(Results[[x]][["Result"]] == "England", na.rm = TRUE), USE.NAMES = TRUE), decreasing = TRUE), main = "Predicted number of England Wins", col = c("red", "white"), las = 2, cex.names = 0.7, ylim = c(0,3), yaxp=c(0, 3, 3))
abline(h = sum(Results$CorrectResults[["Result"]] == "England"), lty = 2, col = "black")

#Wales wins
barplot(sort(sapply(names(Results[1:length(Results)]), function(x) sum(Results[[x]][["Result"]] == "Wales", na.rm = TRUE), USE.NAMES = TRUE), decreasing = TRUE), main = "Predicted number of Wales Wins", col = c("red", "green"), las = 2, cex.names = 0.7, ylim = c(0,3), yaxp=c(0, 3, 3))
abline(h = sum(Results$CorrectResults[["Result"]] == "Wales"), lty = 2, col = "black")

#Scotland wins
barplot(sort(sapply(names(Results[1:length(Results)]), function(x) sum(Results[[x]][["Result"]] == "Scotland", na.rm = TRUE), USE.NAMES = TRUE), decreasing = TRUE), main = "Predicted number of Scotland Wins", col = c("white", "blue"), las = 2, cex.names = 0.7, ylim = c(0,3), yaxp=c(0, 3, 3))
...
#Italy wins
barplot(sapply(names(Results[1:length(Results)]), function(x) sum(Results[[x]][["Result"]] == "Italy", na.rm = TRUE), USE.NAMES = TRUE), main = "Predicted number of Italy Wins", col = c("green", "white", "red"), las = 2, cex.names = 0.7, ylim = c(0,3), yaxp=c(0, 3, 3))
abline(h = sum(Results$CorrectResults[["Result"]] == "Italy"), lty = 2, col = "black")

#draws
barplot(sort(sapply(names(Results[1:length(Results)]), function(x) sum(Results[[x]][["Result"]] == "Draw", na.rm = TRUE), USE.NAMES = TRUE), decreasing = TRUE), main = "Predicted number of Draws", col = viridis::turbo(length(Results)), las = 2, cex.names = 0.7)
abline(h = sum(Results$CorrectResults[["Result"]] == "Draw"), lty = 2, col = "black")

#total goals
barplot(sort(sapply(names(Results[1:length(Results)]), function(x) sum(sum(as.numeric(Results[[x]][["HomeScore"]]), na.rm = TRUE), sum(as.numeric(Results[[x]][["AwayScore"]]), na.rm = TRUE), na.rm = TRUE), USE.NAMES = TRUE), decreasing = TRUE), main = "Predicted number of Goals (Total)", col = viridis::viridis(length(Results)), las = 2, cex.names = 0.7)
```

```{r GroupWinners, eval=FALSE}
#too much coding required to get group tables for each person so I gave up.
Results$CorrectResults %>% group_by(Home) %>% ifelse(Result == Home, 3, ifelse(Result == "Draw", 1, 0))

sapply(names(Results[1:length(Results)]), function(x) sum(Results[[x]][["Result"]] == "England", na.rm = TRUE), USE.NAMES = TRUE)
```
...
ScoresMat<- cbind(ScoresMat, Guess = paste0("Guess: ",Results[[P]][,2], "-", Results[[P]][,3]))
...
Quickly see the groupthink for the next few games.

```{r upcominggames, fig.show='hold', fig.height=0.75, out.width='100%', results='hide', warning=FALSE, eval=FALSE}
DoneGames<- which(!is.na(Results$CorrectResults$HomeScore))

UpcomingGames<- lapply(1:4, function(y) lapply(Results, function(x) head(x[-DoneGames,"Result"], n = 4)[y])) #remove donegames from results dataframes and get the next 4 games from each dataframe (using head n=4)

UpcomingGame1<- as.data.frame(table(unlist(UpcomingGames[[1]]))) %>% arrange(Freq)
UpcomingGame2<- as.data.frame(table(unlist(UpcomingGames[[2]]))) %>% arrange(Freq)
UpcomingGame3<- as.data.frame(table(unlist(UpcomingGames[[3]]))) %>% arrange(Freq)
UpcomingGame4<- as.data.frame(table(unlist(UpcomingGames[[4]]))) %>% arrange(Freq)#get counts of the three options from all dataframes

#horizontal barplot with proportions of home/draw/away
library(ggrepel)
lapply(list(UpcomingGame1, UpcomingGame2, UpcomingGame3, UpcomingGame4), function(x) ggplot(data = x, aes(y = Freq, x = 1, fill = Var1, label = Var1)) + 
         geom_bar(stat = "identity", position = position_stack(reverse = TRUE)) + 
         geom_text(colour = "white", position = position_stack(reverse = TRUE), stat = "identity", hjust = "inward") +
         theme_void() + 
...
```{r HallofFame}
#Euros 2024
WC2026<- data.frame(Position = c(1,2,3), Player = c("Could", "be", "you"))

#Euros 2024
Euro2024<- data.frame(Position = c(1,2,3), Player = c("Paul Newcomb", "Martin Newcomb", "Ben Flynn"))
...
kbl(WC2018, caption = "World Cup 2018", align = "l") %>% kable_classic(html_font = "Cambria") %>%  row_spec(1, bold = T, background = "#c9b037") %>% row_spec(2, bold = T, background = "#b4b4b4") %>% row_spec(3, bold = T, background = "#ad8a56")
...
kbl(WC2026, caption = "World Cup 2026", align = "l") %>% kable_classic(html_font = "Cambria") %>%  row_spec(1, bold = T, background = "#c9b037") %>% row_spec(2, bold = T, background = "#b4b4b4") %>% row_spec(3, bold = T, background = "#ad8a56")
...
#EXTRA BITS TO INCLUDE NEXT TIME AROUND
  ...
  #Include date in Allresults.rds so that a filter-by-date can be added into resultsapp.r
...
This webpage and the predictor game is maintained by Dr Ben Gillard. If you spot any bugs or have any ideas for improvements please contact me at benspredictorgame@gmail.com
