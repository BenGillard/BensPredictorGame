...
UpcomingGames<- lapply(1:3, function(y) lapply(Results, function(x) head(x[-DoneGames,"Result"], n = 3)[y])) #remove donegames from results dataframes and get the next 3 games from each dataframe (using head n=3)
...
UpcomingGame3<- as.data.frame(table(unlist(UpcomingGames[[3]]))) %>% arrange(Freq)#get counts of the three options from all dataframes

#horizontal barplot with proportions of home/draw/away
library(ggrepel)
lapply(list(UpcomingGame1, UpcomingGame2, UpcomingGame3), function(x) ggplot(data = x, aes(y = Freq, x = 1, fill = Var1, label = Var1)) + 
