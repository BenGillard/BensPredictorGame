...
We have **`r length(Results)-3` players** involved so we have a healthy kitty to pay out for the **top 5 players. £`r round(x = (length(Results)-3)*5*0.45, digits = 2)` goes to the top dog. £`r round(x = (length(Results)-3)*5*0.25, digits = 2)` for the top loser. £`r round(x = (length(Results)-3)*5*0.15, digits = 2)` for the best of the rest. And £`r round(x = (length(Results)-3)*5*0.05, digits = 2)` for 4th, 5th, and 6th places.**
