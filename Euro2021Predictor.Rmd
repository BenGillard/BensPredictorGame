...
We have `r length(Results)-2` players involved so we have a healthy kitty to pay out for the top 3 players. £`r (length(Results)-2)*5*0.5` goes to the top dog. £`r (length(Results)-2)*5*0.3` for the top loser. And £`r (length(Results)-2)*5*0.2` for the best of the rest.
...
kbl(LeaderBoard, caption = "Table of Champions", align = "l", row.names = TRUE) %>% kable_classic(html_font = "Cambria") %>%  row_spec(1, bold = T, background = "#c9b037") %>% row_spec(2, bold = T, background = "#b4b4b4") %>% row_spec(3, bold = T, background = "#ad8a56") %>% row_spec(which(LeaderBoard$Player == "RandomControlEntry"), italic = T, color = "red")
```
...
Just a couple of plot showing who we, as a collective of Mystic Megs, predict will be the winners and losers in the group stage. How many times have we predicted a win for each team?
...
Specifically, how many times does each player think the home nations will win in the group stage  (more teams available on request)? And how many boring draws do people think we'll suffer?
...
ResultsMat<- cbind(ResultsMat, Game = paste(ResultsMat$...3, "-", ResultsMat$...6)) #create a column combining the match teams into one
...
ResultsMat[1]<- NULL #remove dates
